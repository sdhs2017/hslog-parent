 ...
 * @author
 */
public class ZabbixUtil2 {

    private static final Logger LOGGER = LoggerFactory.getLogger(ZabbixUtil2.class);

    private ZabbixApi zabbixApi;

    public ZabbixUtil2(String username, String password, String url) throws Exception {
     ...
     * @return
     */
    public String getAlertList_timefrom(Long timeFrom) throws Exception {
    ...
    /**
     * 获取zabbix报警列表，截止时间
     * @param timetill 仅返回在给定时间之前生成的警报。
     * @return
     */
    public String getAlertList_timetill(Long time_till) throws Exception {
    	Request request = RequestBuilder.newBuilder().method("alert.get")
    			.paramEntry("output", new String[]{"sendto", "subject", "clock", "message"})
    			.paramEntry("selectHosts", new String[]{"host"})
    ...
    public String getAlertListByGroupidsWithoutTime(Integer groupid) throws Exception {
    	Request request = RequestBuilder.newBuilder().method("alert.get")
    			.paramEntry("groupids", groupid)
    			.paramEntry("output", new String[]{"sendto", "subject", "clock", "message","triggerid"})
    			.paramEntry("selectHosts", new String[]{"host"})
