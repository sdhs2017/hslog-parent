package com.jz.bigdata.components.kafka.test.test1;
...
//		props.put("zookeeper.connect", "192.168.64.128:2181");
		props.put("zookeeper.connect", "192.168.100.101:2181");
		//group 代表一个消费组
...
//		MyDB mydb= new MyDB();
//		while (it.hasNext())
//		mydb.insertClassify(new Date(),it.next().message().toString()
//				);
//		  try {
//			    return true;
//			    } catch(Exception e) {
...
//
//		Map<String, Integer> topicCountMap = new HashMap<String, Integer>();
//		topicCountMap.put(CreateData.TOPIC1, new Integer(1));
//		//设置译码器
//		StringDecoder keyDecoder = new StringDecoder(new VerifiableProperties());
//		StringDecoder valueDecoder = new StringDecoder(new VerifiableProperties());
//
...
//				consumer.createMessageStreams(topicCountMap,keyDecoder,valueDecoder);
//		//获取数据
//		KafkaStream<String, String> stream = consumerMap.get(CreateData.TOPIC1).get(0);
//		ConsumerIterator<String, String> it = stream.iterator();
//		while (it.hasNext()){
...
//			System.err.println(equ.getId()+equ.getUserId()+equ.getDepartmentId());
			...
			while (it.hasNext()) {
				// Pvuv pvuv =
				// JSON.parseObject(it.next().message().toString(),Pvuv.class);
				// System.out.println(mydb.insertGoods(pvuv));
				System.out.println(it.next().message().toString());
				//Syslog syslog = new Syslog(it.next().message().toString());
...
//      }
	...
	public static void main(String[] args) throws SQLException, InterruptedException {
		KafkaConsumer m = new KafkaConsumer(null, null);
