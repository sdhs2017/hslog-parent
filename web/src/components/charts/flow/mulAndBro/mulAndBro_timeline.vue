    ...
    export default {
        ...
        methods:{
            ...
            getEchartData(params){
                this.$nextTick( ()=> {
                    this.$axios.post(this.$baseUrl+'/flow/getMulticastAndBroadcastPacketTypeCount_line.do',this.$qs.stringify(params))
                        .then((res) => {
                            this.loading = false;
                            let allObj = res.data;
                            if(allObj.success === 'true'){
                                ...
                                console.log(this.timeLineData.data )
                            }else{
                                this.errState = true;
