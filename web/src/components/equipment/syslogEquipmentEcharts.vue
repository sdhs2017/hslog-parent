...
<script>
    import vEchartsTable from '../equipment/syslogEchartsTable';
    import bus from '../common/bus';
    import {downloadToPDF} from  '../../../static/js/common.js'
    export default {
        name: "syslogEquipmentEcharts",
        data(){
            return{
                equipmentId:'',//资产id
                equipmentName:'',//资产名称
        ...
        beforeRouteEnter(to, from, next) {
            next (vm => {
                ...
                //修改此组件的name值
                vm.$options.name = 'syslogEquipmentEcharts'+ to.query.id;
                //修改data参数
                vm.equipmentName = to.query.name;
                vm.busName = 'syslogEquipmentEcharts'+to.query.id;
                //将路由存放在本地 用来刷新页面时添加路由
                let obj = {
                    path:'syslogEquipmentEcharts'+to.query.id,
                    component:'equipment/syslogEquipmentEcharts.vue',
                    title:'统计'
                }
                sessionStorage.setItem('/syslogEquipmentEcharts'+to.query.id,JSON.stringify(obj))
