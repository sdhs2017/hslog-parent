    ...
    export default {
        ...
        methods:{
            ...
            getAssetGroup(){
                this.$nextTick(()=>{
                    //this.loading = true;
                    this.$axios.post(this.$baseUrl+'/assetGroup/getAssetGroupList.do',this.$qs.stringify())
                        .then(res=>{
                            let obj = res.data;
                            if(obj.success === 'true'){
                                this.groupOpt = obj.data;
                                this.formConditionsArr[this.formConditionsArr.length - 2].options = this.groupOpt;
                            }else{
...
/*                            this.$nextTick(()=>{
            ...
            /*跳转到添加资产页面*/
            goToAddEquipment(){
                this.$router.push({path:'addEquipmentWithGroup'});
            },
            /*跳转到所有资产报表*/
            goToAllEcharts(){
            ...
            removeEquipment(){
                if(this.checkList.length === 0){
                    ...
                    layer.confirm('您确定删除么？', {
                        ...
                        this.$nextTick(()=>{
                            this.$axios.post(this.$baseUrl+'/equipment/deleteWithAssetGroup.do',this.$qs.stringify({id:delectEquipmentIds}))
                                .then((res)=>{
            ...
            reviseEquipment(rowData,index){
                //跳转页面
                jumpHtml('reviseEquipmentWithGroup'+rowData.id,'equipment/reviseEquipmentWithGroup.vue',{ name:rowData.name,id: rowData.id },'资产修改')
            },
            /*查看资产日志*/
        ...
        beforeRouteEnter(to, from, next) {
            next (vm => {
                ...
                if(to.params.equipmentid !== undefined){
                    ...
                    vm.selectEquipment();
                    vm.c_page = 1;
                }else if(to.params.groupId !== undefined){
                    vm.searchConditions.asset_group_id = to.params.groupId;
                    vm.formConditionsArr[vm.formConditionsArr.length - 2].model.model = to.params.groupId;
