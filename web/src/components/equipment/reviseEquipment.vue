    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            next (vm => {
                ...
                //修改此组件的name值
                vm.$options.name = 'reviseEquipment'+ to.query.id;
                //修改data参数
                vm.equipmentName = to.query.name;
                vm.busName = 'reviseEquipment'+to.query.id;
                //将路由存放在本地 用来刷新页面时添加路由
                let obj = {
                    path:'reviseEquipment'+to.query.id,
                    component:'equipment/reviseEquipment.vue',
                    title:'资产修改'
                }
                sessionStorage.setItem('/reviseEquipment'+to.query.id,JSON.stringify(obj))
