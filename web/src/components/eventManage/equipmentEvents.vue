    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            next (vm => {
                ...
                //修改此组件的name值
                vm.$options.name = 'equipmentEvents'+ to.query.id;
                //修改data参数
                vm.equipmentName = to.query.name;
                vm.busName = 'equipmentEvents'+to.query.id;
                //将路由存放在本地 用来刷新页面时添加路由
                let obj = {
                    path:'equipmentEvents'+to.query.id,
                    component:'eventManage/equipmentEvents.vue',
                    title:'事件'
                }
                sessionStorage.setItem('/equipmentEvents'+to.query.id,JSON.stringify(obj))
                if(vm.equipmentId === '' || vm.equipmentId !== to.query.id){
                    vm.equipmentId = to.query.id;
                    vm.eventSearchCondition.equipmentid = vm.equipmentId;
                    vm.getEquipmentData();
                }
