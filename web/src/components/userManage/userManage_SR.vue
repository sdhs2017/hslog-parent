                ...
                <el-col :span="6">
                    <div class="department-wapper" @click="removeClass" v-loading="leftLoading"  element-loading-background="rgba(48, 62, 78, 0.5)">
                        <div class="department-top">
                            <h5>机构列表</h5>
                            <div class="department-btns" v-if="$is_has('userManage_departmentBtns')">
        ...
        <el-dialog :title="userBtnType === 'add' ? '添加用户':'修改用户'" :visible.sync="userForm" width="500px" :close-on-click-modal="clickModal">
            <el-form label-width="100px">
                ...
                <el-form-item label="密码:" v-if="userBtnType === 'add'">
                    <span style="color:red;position: absolute;left: -10px;">*</span>
                    <el-input v-model="userParams.password" type="password" :placeholder="paw_placeholder"  maxlength="18"  class="item"></el-input>
                </el-form-item>
...
*/
        ...
        methods:{
            ...
            getTreeData(params,resolve){
                this.depData=[]
                this.$nextTick(()=>{
                    this.$axios.post(this.$baseUrl+'/department/selectAll_Security.do',this.$qs.stringify(params))
                        .then(res =>{
                            //判断数据是否为空
                            if(res.data !== ''){
                                //加载组织机构列表
            ...
            selectUser(page,obj){
                ...
                this.rightLoading = true;
                this.$nextTick(()=>{
                    this.$axios.post(this.$baseUrl+'/user/selectAll_Security.do',this.$qs.stringify(obj))
                        .then(res =>{
                            this.rightLoading = false;
            ...
            addUser(){
                ...
                if(this.checkUserParams(this.userParams)){
                    layer.load(1)
                    this.$nextTick(()=>{
                        this.$axios.post(this.$baseUrl+'/user/insert_Security.do',this.$qs.stringify(this.userParams))
                            .then(res =>{
                                layer.closeAll('loading')
                                if(res.data.success === 'true'){
                                    layer.msg('添加成功',{icon: 1})
            ...
            checkUserParams(params){
                if(this.userBtnType === 'add'){
                    ...
                    if(params.password !== this.password2){
                        layer.msg('两次密码不正确',{icon: 5});
                        return false;
                    }
                }
