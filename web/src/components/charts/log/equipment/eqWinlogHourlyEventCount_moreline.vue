<template>
    ...
    <div class="eb" v-loading="loading"  element-loading-background="rgba(48, 62, 78, 0.5)">
        ...
        <v-echarts v-else echartType="line" :echartData = "this.moreLineData" :busName="busName" ></v-echarts>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            getEchartData(params){
                this.$nextTick( ()=> {
                    this.$axios.post(this.$baseUrl+'/log/getCountGroupByTimeAndEvent_line.do',this.$qs.stringify(params))
                        .then((res) => {
                            this.loading = false;
                            let obj = res.data;
                            if(obj.success === 'true'){
